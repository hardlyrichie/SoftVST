...
public:
    ...
    CabSimulator()
    {
        ...
        // Here, we are specifying the impulse response that the convolver will use 
        // to reverberate the incoming signal.

        while (!dir.getChildFile("Resources").exists() && numTries++ < 15)
            dir = dir.getParentDirectory();

        // We initialise the convolver with the guitar amp impulse response by 
        // loading the audio file from the Resources folder[6]
...
private:
    //==============================================================================
    // Using convolution to simulate a guitar cabinet
...
public:
    ...
    Distortion()
    {
        // Introducing a tanh function based distortion filter
        ...
        // Here we boost the sound pre-processing by 30dB
        auto& preGain = processorChain.template get<preGainIndex>();   // [5]
        preGain.setGainDecibels(30.0f);                               // [6]

        // Then we reduce the boost by 20dB for the final output
        auto& postGain = processorChain.template get<postGainIndex>(); // [7]
        // postGain.setGainDecibels(-20.0f);                             // [8]

        // The postGain is removed for the guitar cabinet simulator, since
        // the signal will get attenuated naturally through the convolution 
        // process which happens after the distortion in our signal chain
...
private:
    ...
    // Here, we are introducing a high-pass filter since when distorting low frequency 
    // content with the waveshaper, the distorted sound can get muddy very quickly. 
    // We can alleviate this problem by introducing a high-pass filter before processing
    // the signal with the waveshaper.
    ...
    void renderNextBlock (AudioBuffer<float>& outputBuffer, int startSample, int numSamples) override
    {
        for (int i = 0; i < numSamples; ++i)
        {
            if (--lfoProcessingIndex == 0)
            {
                ...
                auto cutoffHz = jmap (lfoOut, -1.0f, 1.0f, 100.0f, 4e3f);
